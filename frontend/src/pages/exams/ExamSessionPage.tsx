...
const AUTOSUBMIT_TIME = 3 * 1000; // 3 seconds before end
...
const ExamSessionPage = () => {
  ...
  // Save/load session data to/from session storage
  ...
  useEffect(() => {
    ...
    const updateTimer = () => {
      ...
      // Auto-submit before end
      if (remaining <= AUTOSUBMIT_TIME && remaining > 0) {
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogAction
              ...
              onClick={() => navigate(`/course/${courseId}?tab=exams`)}
            >
              Return
            </AlertDialogAction>
          </AlertDialogFooter>
